...
reduceScrollareaHeight:function(id, data)
 ...
 {
 if (newHeight >0)
 prevSibling.style.height = newHeight + "px"; else if (prevSibling.height > 228)
 ...
 parentWin = getFirstParentWin(); url += '?' + this.paramsModal; this.SetInProcess(false); this.SetWaitingObject(null,"",null,false,false); closeModalAll(); parentWin.modWin = null; parentWin.doModal(url, this.bPrompt, parentWin, sCancelName, this.form, this.id); return; }
 ...
 winParent.doModal(url + '?' + this.paramsModal, this.bPrompt, winParent, sCancelName, this.form, this.id); return; }
 }
 else {
 if (bModal == 4)
 this.closeModal(); nResubmit = 0; loader = null;  doModal(url + '?' + this.paramsModal, this.bPrompt, window, sCancelName, this.form, this.id, this.req.responseText); return; }
 } 
 if (bModal == 2)
 {
